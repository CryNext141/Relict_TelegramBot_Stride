    ...
    public class BotService : IBotService
    {
        ...
        private async Task HandleCallback(CallbackQuery cb, CancellationToken ct, bool answer = true)
        {
            ...
            if (cb.Data == "menu")
            {
                ...
                if (_cache.TryGetValue($"menu_{chatId}", out int oldMenu))
                    await SafeDelete(Client, chatId, oldMenu, ct);
                if (_cache.TryGetValue($"about_{chatId}", out int aboutMsg))
                {
                    await SafeDelete(Client, chatId, aboutMsg, ct);
                    _cache.Remove($"about_{chatId}");
                }
                if (_cache.TryGetValue($"faq_{chatId}", out int faqMsg))
                {
                    await SafeDelete(Client, chatId, faqMsg, ct);
            ...
            if (cb.Data == "about_bot")
            {
                if (_cache.TryGetValue($"menu_{chatId}", out int mainMenuMsgId))
                {
                    await SafeDelete(Client, chatId, mainMenuMsgId, ct);
        ...
        private async Task ShowAboutBot(long chatId, CancellationToken cancellationToken)
        {
            var aboutText = "У чат-боті «Stride» громадяни мають можливість:\n\n" +
              "🔹 Переглядати активні алерти викрадення дітей;\n" +
              "<code>Отримувати детальну інформацію з фотографіями та описом випадку</code>\n\n" +
              "🔹 Надсилати свідчення про події викрадення;\n" +
              "<code>Можливість оперативно повідомляти про важливі дані (з опцією анонімності)</code>\n\n" +
              "🔹 Підписуватися на сповіщення в реальному часі.\n" +
              "<code>Керувати налаштуваннями підписок для отримання сповіщень про нові викрадення</code>";
            var aboutMsg = await Client.SendMessage(
                chatId,
                aboutText,
                parseMode: ParseMode.Html,
                replyMarkup: InlineMenus.AboutNav(),
                cancellationToken: cancellationToken);
            _cache.Set($"about_{chatId}", aboutMsg.MessageId);
            return;
        }

        private async Task ShowFaq(long chatId, CancellationToken cancellationToken)
        {
            
            var faqMsg = await Client.SendMessage(
                chatId,
                faqText,
                parseMode: ParseMode.Html,
                replyMarkup: InlineMenus.FaqNav(),
                cancellationToken: cancellationToken);
            _cache.Set($"faq_{chatId}", faqMsg.MessageId);
        }
        ...
        {
            bool isClosed = text.Contains("was closed", StringComparison.OrdinalIgnoreCase) ||
                            text.Contains("закрит", StringComparison.OrdinalIgnoreCase);
            string icon = isClosed ? "✅" : "🚨";

            string finalText = $"{icon} *Алерт #{alertId}*\r\n{text}";

            IReplyMarkup? markup = isClosed
                ? null
                : new InlineKeyboardMarkup(
                      InlineKeyboardButton.WithCallbackData("ℹ️ Детальніше",
        ...
        private readonly string faqText = "Чат-бот «Stride» — ваш помічник для отримання інформації про важливі події та надсилання повідомлень про інциденти.\n\n" +
        "🔹 **Що таке чат-бот «Stride»?**\n" +
        "«Stride» — це чат-бот, який допомагає тримати вас у курсі важливих подій та надає можливість повідомляти про інциденти.\n\n" +
        "🔹 **Для чого потрібен бот «Stride»?**\n" +
        "Бот дозволяє швидко отримувати інформацію про актуальні ситуації та надсилати власні повідомлення, якщо ви стали свідком події.\n\n" +
        "🔹 **Як почати використовувати бота?**\n" +
        "Для початку достатньо надіслати команду «/start». Бот відповість повідомленням з головним меню, з якого можна обрати необхідну дію.\n\n" +
        "🔹 **Як я можу отримувати сповіщення?**\n" +
        "Бот надсилає сповіщення про нові події, щоб ви завжди були в курсі важливих змін та інцидентів.\n\n" +
        "🔹 **Як переглянути детальну інформацію про подію?**\n" +
        "Ви можете обрати інцидент зі списку, після чого бот надасть розгорнуту інформацію, включаючи фотографії та опис.\n\n" +
        "🔹 **Як надіслати своє повідомлення про подію?**\n" +
        "Бот пропонує покрокову інструкцію, де ви можете вказати деталі події, контакти та місцезнаходження.\n\n" +
        "🔹 **Чи можна залишатися анонімним при надсиланні повідомлень?**\n" +
        "Так, бот дозволяє обрати режим анонімності, якщо ви не бажаєте розкривати свої особисті дані.\n\n" +
        "🔹 **Чи потрібен мені номер телефону для користування ботом?**\n" +
        "Номер телефону може бути запитаний під час надсилання повідомлень, але ви маєте можливість обрати анонімний режим.\n\n" +
        "🔹 **Чи можу я поділитись своєю геолокацією?**\n" +
        "Так, бот може запросити вашу геолокацію для більш точного визначення місця події.\n\n" +
        "🔹 **Що робити, якщо я не отримую сповіщень?**\n" +
        "Переконайтеся, що ви підписані на сповіщення та перевірте налаштування бота. Можна повернутися до головного меню і повторити процес.\n\n" +
        "🔹 **Як працює головне меню?**\n" +
        "Головне меню надає основні варіанти: перегляд подій, надсилання повідомлень та управління підписками на сповіщення.\n\n" +
        "🔹 **Чи є бот безпечним у використанні?**\n" +
        "Бот розроблено з думкою про вашу безпеку, а всі дані передаються через захищені канали.\n\n" +
        "🔹 **Як забезпечується конфіденційність моїх даних?**\n" +
        "Ваші дані використовуються лише для відповідних операцій і не розголошуються третім сторонам.\n\n" +
        "🔹 **Чи можна змінити налаштування підписки?**\n" +
        "Так, ви можете вводити зміни, відписуватися або додавати нові сповіщення у спеціальному меню.\n\n" +
        "🔹 **Для чого потрібні повідомлення про події?**\n" +
        "Ці повідомлення дають можливість бути інформованим про важливі інциденти та своєчасно реагувати на них.\n\n" +
        "🔹 **Як часто надходять сповіщення?**\n" +
        "Сповіщення надходять у режимі реального часу або з короткими інтервалами, залежно від ситуації.\n\n" +
        "🔹 **Чи можу я відписатися від сповіщень?**\n" +
        "Так, у меню підписок є можливість відписатися від небажаних сповіщень.\n\n" +
        "🔹 **Що робити, якщо бот не відповідає?**\n" +
        "Спробуйте перезапустити бот, використавши команду «/start», або зверніться за допомогою до адміністратора.\n\n" +
        "🔹 **Як звернутися за допомогою?**\n" +
        "За додатковою інформацією або підтримкою можна звернутися за контактним номером, який вказаний у боті.\n\n" +
        "🔹 **Чи потрібні додаткові установки для роботи з ботом?**\n" +
        "Ні, бот працює без додаткових програм. Все, що вам потрібно, — це Telegram.\n\n" +
        "🔹 **Чи можу я використовувати бота на будь-якому пристрої?**\n" +
        "Так, бот доступний в Telegram, тому ви можете користуватися ним з телефону, планшета чи комп'ютера.\n\n" +
        "🔹 **Який основний функціонал бота?**\n" +
        "Основний функціонал полягає в оперативному інформуванні про важливі події та дає можливість повідомити про них за допомогою зручного інтерфейсу.";
